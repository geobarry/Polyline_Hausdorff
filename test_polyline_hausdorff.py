    ...
    def test_transform_matches(self):
        ...
        for itnum in range(self.numits):
            ...
            for s in range(case(-1)):
                ...
                Hprime,srclocprime,trglocsprime = h.polyline_hausdorff(Aprime, Bprime)
                self.assertAlmostEqual(H, Hprime)
                # check the Hausdorff distance from each segment
                def segHausdorff_wrapper(C,D,seg):
                    startNearSeg = hu.nearSegment(C, D, seg)
                    endNearSeg = hu.nearSegment(C, D, seg+1)
                    startcomp, startd = hu.nearComponent(C, D, seg, startNearSeg)
                    endcomp, endd = hu.nearComponent(C, D, seg+1, endNearSeg)
                    candidates = hu.candidateComponents(C,D,seg)
                    d,k,c1,c2 = h.hausdorff_segment(C, D, seg, startcomp, startd, endcomp, endd, candidates)
                    return d,k,c1,c2
                def make_comparison(A,B,Aprime,Bprime,seg):
                    # original
                    d,k,c1,c2 = segHausdorff_wrapper(A,B,seg)
                    # transformed
                    dprime,kprime,c1prime,c2prime = segHausdorff_wrapper(Aprime,Bprime,seg)
                    # compare
...
def test_hausdorff_segment(A,B,seg,verbose=False, titleprefix=""):    
    startNearSeg = hu.nearSegment(A, B, seg)
    endNearSeg = hu.nearSegment(A, B, seg+1)
    startcomp, startd = hu.nearComponent(A, B, seg, startNearSeg)
    endcomp, endd = hu.nearComponent(A, B, seg+1, endNearSeg)
    candidates = hu.candidateComponents(A,B,seg)
    d,k,c1,c2 = h.hausdorff_segment(A, B, seg, startcomp, startd, endcomp, endd, candidates,verbose)
    msg = "Aseg{} ".format(seg)
    if c2==None:
        msg += "to B{}".format(hu.component_label(c1))
    else:
        msg += "to B{},B{}".format(hu.component_label(c1),hu.component_label(c2))
